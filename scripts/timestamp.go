...
func TimeStamp(d int) {
	...
	begin
		for i in 1 .. array_upper(dates, 1) loop
			IF NOT EXISTS (select 1 from information_schema.tables where table_name='{{.TableName}}_' || sdates[i] ) THEN
				...
				execute format('alter table {{.TableName}}_%s add constraint ck check(to_timestamp({{.CheckItem}})::date::text = ''%s'')', sdates[i], dates[i]);
			END IF;
		end loop;
	end;  
	$$;
	...
	begin`, tableName, tableName, checkItem)
	  ...
	  check_date := to_timestamp(NEW.%s)::date::text;
      sdate := to_char(to_timestamp(NEW.%s)::date,'yyyymmdd');
      case check_date
			%s
	  end case;
	  return null;`, checkItem, checkItem, getTimeStampCaseStr(tableName, dateList, shortDateList))
...
func getTimeStampCaseStr(tableName string, DateList, shortDateList []string) string {
	var ceseStr string
	...
	ceseStr += `
          ...
          THEN
				...
				execute format('alter table %s_%s add constraint ck check(to_timestamp(%s)::date::text = ''%s'')', tablename,sdate,checkitem, check_date);
		  END IF;
